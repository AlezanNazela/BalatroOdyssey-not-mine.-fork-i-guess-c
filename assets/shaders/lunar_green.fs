...
extern MY_HIGHP_OR_MEDIUMP vec2 lunar_green;
...
// Kept to avoid errors
number hue(number s, number t, number h){return 0.;}
vec4 RGB(vec4 c){return c;}
vec4 HSL(vec4 c){return c;}
float rand(vec2 co){ return 0.0; }
...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
    ...
    // --- 1. Find Distance to Nearest Edge ---
    ...
    float pulse = 0.95 + 0.05 * sin(time * 3.0 + lunar_green.x);
    vec3 neon_green = vec3(0.0, 0.9, 0.3) * 0.9 * pulse; // Subtle green glow

    vec4 final_pixel = vec4(0.0);

    if (sprite_alpha > 0.5) {
        // --- This pixel is INSIDE the card ---
        // Invert to black (white becomes black, colors inverted)
        vec3 inverted_color = 1.0 - original_pixel.rgb;
        final_pixel = vec4(inverted_color, 1.0);

    } else {
        // --- This pixel is OUTSIDE the card (edges) ---
        float glow = 1.0 - (distance_to_edge / float(search_radius));
        glow = pow(max(0.0, glow), 3.0); // Soft glow falloff
        final_pixel = vec4(neon_green * glow * 0.5, glow * 0.5); // Reduced intensity
    }
