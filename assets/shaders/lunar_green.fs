...
vec4 dissolve_mask(vec4 tex, vec2 texture_coords, vec2 uv)
{
    ...
    float t = time * 10.0 + 2003.0 + lunar_green.x * 0.001;
...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
    ...
    float bright_val = (tex.r + tex.g + tex.b) / 3.0;

    if (bright_val > 0.6) {
        vec2 center_uv = uv - vec2(0.5);
        float d1 = length(center_uv) - 0.25;
        float d2 = length(center_uv - vec2(0.1, 0.0)) - 0.25;
        float moon_mask = smoothstep(0.01, 0.0, max(d1, -d2));
        tex.rgb = mix(tex.rgb, vec3(0.65, 0.65, 0.7), moon_mask * 0.4);
    } else {
        tex.rgb = suit_color;
    }
