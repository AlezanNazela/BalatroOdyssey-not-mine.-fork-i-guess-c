...
extern MY_HIGHP_OR_MEDIUMP vec2 lunar_red;
...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
    ...
    float pulse = 0.95 + 0.05 * sin(time * 3.0 + lunar_red.x);
    vec3 neon_red = vec3(1.0, 0.2, 0.3) * 0.9 * pulse; // Subtle red glow 
    ...
    if (sprite_alpha > 0.5) {
        ...
        final_pixel = vec4(neon_red * glow * 0.5, glow * 0.5); // Reduced intensity
    }
